...
@inline function state_update_outer(globaldata, configData, itm, max_res, sum_res_sqr, U)
    nx = globaldata[itm].nx
    ny = globaldata[itm].ny
...
@inline function state_update_interior(globaldata, itm, max_res, sum_res_sqr, U)
    nx = globaldata[itm].nx
    ny = globaldata[itm].ny
    primitive_to_conserved(globaldata, itm, nx, ny, U)
    # if itm == 1
