function cal_flux_residual_kernel(gpuGlobalDataConn, gpuGlobalDataConnSection, gpuGlobalDataFauxFixed, gpuGlobalDataRest, gpuConfigData, numPoints)
	...
	if idx > 0 && idx <= numPoints
		...
		if gpuGlobalDataFauxFixed[idx + 4 * numPoints] == gpuConfigData[17]
			wall_kernel(gpuGlobalDataConn, gpuGlobalDataConnSection, gpuGlobalDataFauxFixed, gpuGlobalDataRest, gpuConfigData, numPoints, shared, flux_shared, qtilde_shared)
		elseif gpuGlobalDataFauxFixed[idx + 4 * numPoints] == gpuConfigData[18]
			interior_kernel(gpuGlobalDataConn, gpuGlobalDataConnSection, gpuGlobalDataFauxFixed, gpuGlobalDataRest, gpuConfigData, numPoints, shared, flux_shared, qtilde_shared)
		elseif gpuGlobalDataFauxFixed[idx + 4 * numPoints] == gpuConfigData[19]
			outer_kernel(gpuGlobalDataConn, gpuGlobalDataConnSection, gpuGlobalDataFauxFixed, gpuGlobalDataRest, gpuConfigData, numPoints, shared, flux_shared, qtilde_shared)
		end
