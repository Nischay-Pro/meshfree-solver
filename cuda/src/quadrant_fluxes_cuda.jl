...
function flux_quad_GxII_kernel_comment(nx, ny, idx, shared, op::Function)
    ...
    shared[thread_idx + block_dim * 3] = op((rho*A2neg*(temp2 + temp3) - temp4), shared[thread_idx + block_dim * 3])
    if idx == 3
        @cuprintf("\n => %.17f %.17f %.17f %.17f", shared[thread_idx], shared[thread_idx + block_dim * 1],
                    shared[thread_idx + block_dim * 2], shared[thread_idx + block_dim * 3])
    end
    return nothing
end
