function wall_dGx_pos_kernel(gpuGlobalDataCommon, idx, gpuConfigData, Gxp, phi_i, phi_k, G_i, G_k,
    ...
    for iter in 56:75
        ...
        qtilde_i =  (
                        gpuGlobalDataCommon[39, idx] - 0.5*(delx * gpuGlobalDataCommon[43, idx] + gpuConfigData[47, idx]),
                        gpuGlobalDataCommon[40, idx] - 0.5*(delx * gpuGlobalDataCommon[44, idx] + gpuConfigData[48, idx]),
                        gpuGlobalDataCommon[41, idx] - 0.5*(delx * gpuGlobalDataCommon[45, idx] + gpuConfigData[49, idx]),
                        gpuGlobalDataCommon[42, idx] - 0.5*(delx * gpuGlobalDataCommon[46, idx] + gpuConfigData[50, idx])
                    )
        qtilde_k = (
                        gpuGlobalDataCommon[39, conn] - 0.5*(delx * gpuGlobalDataCommon[43, conn] + gpuConfigData[47, conn]),
                        gpuGlobalDataCommon[40, conn] - 0.5*(delx * gpuGlobalDataCommon[44, conn] + gpuConfigData[48, conn]),
                        gpuGlobalDataCommon[41, conn] - 0.5*(delx * gpuGlobalDataCommon[45, conn] + gpuConfigData[49, conn]),
                        gpuGlobalDataCommon[42, conn] - 0.5*(delx * gpuGlobalDataCommon[46, conn] + gpuConfigData[50, conn])
                    )
        ...
        if limiter_flag == 1
            ...
            qtilde_i =  (
                            gpuGlobalDataCommon[39, idx] - 0.5*phi_i[1]*(delx * gpuGlobalDataCommon[43, idx] + gpuConfigData[47, idx]),
                            gpuGlobalDataCommon[40, idx] - 0.5*phi_i[2]*(delx * gpuGlobalDataCommon[44, idx] + gpuConfigData[48, idx]),
                            gpuGlobalDataCommon[41, idx] - 0.5*phi_i[3]*(delx * gpuGlobalDataCommon[45, idx] + gpuConfigData[49, idx]),
                            gpuGlobalDataCommon[42, idx] - 0.5*phi_i[4]*(delx * gpuGlobalDataCommon[46, idx] + gpuConfigData[50, idx])
                        )
            qtilde_k = (
                            gpuGlobalDataCommon[39, conn] - 0.5*phi_k[1]*(delx * gpuGlobalDataCommon[43, conn] + gpuConfigData[47, conn]),
                            gpuGlobalDataCommon[40, conn] - 0.5*phi_k[2]*(delx * gpuGlobalDataCommon[44, conn] + gpuConfigData[48, conn]),
                            gpuGlobalDataCommon[41, conn] - 0.5*phi_k[3]*(delx * gpuGlobalDataCommon[45, conn] + gpuConfigData[49, conn]),
                            gpuGlobalDataCommon[42, conn] - 0.5*phi_k[4]*(delx * gpuGlobalDataCommon[46, conn] + gpuConfigData[50, conn])
                        )
        end
